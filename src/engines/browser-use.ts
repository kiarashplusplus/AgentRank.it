...
export class BrowserUseEngine {
  ...
  async analyze(url: string, timeout: number): Promise<BrowserAnalysis> {
    ...
    const robotsTxt = await this.fetchOptionalResource(`${baseUrl.origin}/robots.txt`);
  ...
  private async getAccessibilityTree(): Promise<unknown> {
    ...
    try {
      ...
      const interactiveCount = await this.page
        .locator('button, a, input, select, textarea, [role]')
        .count();
